 ...
 * When a new transaction is detected, the function updates the daily,
 * weekly, and monthly aggregated statistics.
 *
 * @param statsCollection - The collection with individual transaction documents.
 * @param dailyStatsCollection - The collection where daily aggregates are stored.
 * @param weeklyStatsCollection - The collection where weekly aggregates are stored.
 * @param monthlyStatsCollection - The collection where monthly aggregates are stored.
 */
  ...
  setInterval(async () => {
    try {
      ...
      if (
        ...
        // Update daily, weekly, and monthly stats based on this transaction
...
function getWeekKey(date: Date): string {
  ...
  d.setHours(0, 0, 0, 0);
  // Adjust to Thursday in current week — ISO weeks start on Monday, and the week containing Thursday is considered the first week.
...
async function updateDailyStats(
  ...
  // Update (or insert) the daily stats document.
  ...
  // Recalculate and update the average time taken.
...
async function updateWeeklyStats(
  ...
  // Update (or insert) the weekly stats document.
...
async function updateMonthlyStats(
  ...
  // Update (or insert) the monthly stats document.
